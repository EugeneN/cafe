...
{say, shout, scream, whisper} = (require '../lib/logger') "Adaptor/lispy>"
{exec} = require 'child_process'
async = require 'async'

{FILE_ENCODING, TMP_BUILD_DIR_SUFFIX, LISPY_EXT, JS_EXT, 
 LISPY_BIN, CB_SUCCESS} = require '../defs'


get_target_fn = (app_root, module_name) ->
    (path.resolve app_root,
                  TMP_BUILD_DIR_SUFFIX,
                  (module_name.replace (new RegExp "#{LISPY_EXT}$"), '') + JS_EXT)
...
module.exports = do ->
    ...
    match = (ctx) ->
        ...
        (is_file source_fn) and (has_ext source_fn, LISPY_EXT)

    match.async = (ctx, cb) ->
        {source_fn} = get_paths ctx
        async.parallel [((is_file_cb) -> is_file.async source_fn, is_file_cb), 
                        ((is_file_cb) -> has_ext.async source_fn, LISPY_EXT, is_file_cb)],
                        (err, res) ->
    ...
    make_adaptor = (ctx) ->
        ...
        harvest = (cb) ->
            ...
            if ctx.own_args.f or (newer source_fn, target_fn)
                exec "#{LISPY_BIN} #{source_fn} #{target_fn}", (err, stdout, stderr) ->
                    if err

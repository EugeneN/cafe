/* Cafe 0ad40ab6-9be4-4c6d-b204-09697d37efee Mon Dec 03 2012 15:55:27 GMT+0200 (EET) */
/* ZB:genome-parser.js */
...
(function(/*! Stitch !*/) {
  ...
  return this.require.define;
}).call(this)({"JQueryDom": function(exports, require, module) {(function() {
  var say;
...
}, "JqueryDomParser": function(exports, require, module) {
  "Abstraction for Dom elements manipulations\nbased on JQuery.";

  exports.get_by_attr = function(attr, node) {
    if (node) {
      return ($(node)).find(attr);
    } else {
      return $(attr);
    }
  };
  ...
  exports.getData = function(node, attr) {
    return ($(node)).data()[attr];
  };

  exports.get_id = function(node) {
    return ($(node)).attr('id');
  };

  exports.on_dom_ready = ($(document)).ready;

  exports.one = function(sel) {
    return $(sel);
  };

  exports.JQueryDom = require('JQueryDom');
}});
;
/* ZB:dna.js */
//     Underscore.js 1.4.2
...
(function(/*! Stitch !*/) {
  ...
  return this.require.define;
}).call(this)({"dna": function(exports, require, module) {(function() {
  var CELLS, DA_EXTEND, DA_SUBSCRIBE, DEBUG, THIS, dispatch_handler, dispatch_impl, dom_parser, find_cell, get_cell, get_create_cell, get_create_cell_by_id, get_default_protocols, get_protocol, lab, parse_ast_handler_node, parse_genome, register_protocol_impl, save_cell, say, synthesize_cell, _ref,
    __slice = [].slice;
  ...
  _ref = require('libprotocol'), register_protocol_impl = _ref.register_protocol_impl, dispatch_impl = _ref.dispatch_impl, get_protocol = _ref.get_protocol, get_default_protocols = _ref.get_default_protocols;
  ...
  parse_ast_handler_node = function(handler, current_cell, dom_parser) {
    var cell, cell_id, handler_fn, method, ns, partial_args, partially_applied_handler, scope, _ref1;
    if (Array.isArray(handler)) {
      _ref1 = handler[0], ns = _ref1.ns, method = _ref1.method, scope = _ref1.scope;
    } else {
  ...
  lab = function(dom_parser, root_node) {
    var START, cell, cell_matrices, dna_sequences, gene_expression_matrices, node, protocols, _i, _j, _len, _len1;
    say('Cells synthesis started');
    START = new Date;
    if (DEBUG) {
      window.dom_parser = dom_parser;
    }
    cell_matrices = dom_parser.get_by_attr("[data-" + DA_EXTEND + "]", root_node);
    gene_expression_matrices = dom_parser.get_by_attr("[data-" + DA_SUBSCRIBE + "]", root_node);
    for (_i = 0, _len = cell_matrices.length; _i < _len; _i++) {
      node = cell_matrices[_i];
      protocols = ((dom_parser.getData(node, DA_EXTEND)).split(" ")).filter(function(i) {
        return !!i;
      });
    ...
    for (_j = 0, _len1 = gene_expression_matrices.length; _j < _len1; _j++) {
      node = gene_expression_matrices[_j];
      cell = get_create_cell(node.id, node, dom_parser);
      dna_sequences = parse_genome(dom_parser.getData(cell.node, DA_SUBSCRIBE));
      ...
      dna_sequences.map(function(dna_seq) {
        ...
        return dna_seq.events.map(function(events) {
          var args, event, ns, raw_args, scope, _ref1, _ref2;
          _ref1 = Array.isArray(events) ? events : [events, []], (_ref2 = _ref1[0], ns = _ref2.ns, event = _ref2.event, scope = _ref2.scope), raw_args = 2 <= _ref1.length ? __slice.call(_ref1, 1) : [];
          args = (raw_args.filter(function(a) {
            var _ref3;
            return (_ref3 = a.event.type) === 'number' || _ref3 === 'string' || _ref3 === 'vector';
          })).map(function(a) {
  ...
  module.exports = {
    start_synthesis: function(dom_parser) {
      return lab(dom_parser);
    },
    dump_cells: function() {
  ...
  dom_parser = require('init_parser');

  dom_parser.on_dom_ready(function() {
    register_protocol_impl('IDom', dom_parser.JQueryDom);
    lab(dom_parser, dom_parser.one('body'));
    return document.addEventListener("DOMNodeInserted", function(event) {
      return lab(dom_parser, dom_parser.one(event.target));
    });
  });

}).call(this);
}, "init_parser": function(exports, require, module) {(function() {

  module.exports = require('JqueryDomParser');

}).call(this);
}, "DefaultProtocols": function(exports, require, module) {(function() {
  var Math, register_protocol, register_protocol_impl, _ref;

  _ref = require('libprotocol'), register_protocol_impl = _ref.register_protocol_impl, register_protocol = _ref.register_protocol;
  ...
  register_protocol_impl('IMath', Math);

}).call(this);
}, "libprotocol": function(exports, require, module) {(function() {
  var DefaultProtocols, Implementations, Protocols, THIS, dispatch_impl, dump_impls, get_default_protocols, get_protocol, register_protocol, register_protocol_impl, say,
    __slice = [].slice;
  ...
  DefaultProtocols = {
    IDom: [['setContent', ['new_content']], ['setValue', ['new_value']], ['setText', ['new_text']], ['getValue', []], ['alert', ['msg']], ['click', ['handler']], ['keyDown', ['handler']], ['say', ['msgs']], ['appendContent', ['content']], ['kill', []], ['stop_event', ['e']], ['setAttr', ['attr']], ['dbclick', ['e']], ['focusout', ['e']], ['focus', []], ['onInit', ['f']]],
    IMath: [['len', ['array']]]
  };
  ...
  get_protocol = function(p) {
    ...
    if (!Protocols) {
      Protocols = {};
      for (k in DefaultProtocols) {
        v = DefaultProtocols[k];
        Protocols[k] = v;
      }
    }
    if (Protocols.hasOwnProperty(p)) {
      ...
      throw "DNA has no registered protocol '" + p + "'";
    }
  };
  ...
  dispatch_impl = function() {
    ...
    Protocols[protocol] || (function() {
      throw "DNA can't find protocol " + protocol;
    })();

                ...
                if (pkg.main) {
                    loadAsFile(path.resolve(x, pkg.main), pkg, function (err, m, pkg) {
                        ...
                        loadAsDirectory(dir, pkg, function (err, n, pkg) {
                            ...
                            var main_file_is_dir_load_index_cb = function(err, m, pkg){
                                if (err) return cb(err);
                                if (!m) {
                                    return cb("resolve: package.json's main file not found in dir " + dir); }
                                else {
                                    return cb(err, m, pkg);
                                }
                            };

                            loadAsFile(path.join(x, '/index'), pkg, main_file_is_dir_load_index_cb);
                        });
                    });
                    return;
                }

...
(function() {
  var SEP, SUB_CAFE, THRESHOLD_INTERVAL, THRESHOLD_QUEUE_ACTIVE, THRESHOLD_QUEUE_EMPTY, WATCH_FN_PATTERN, build, build_cmd, chokidar, filter_dict, fork, is_array, is_debug_context, maybe_build, path, path_matches, say, scream, shout, spawn, whisper, _get_build_dir, _ref, _ref1, _ref2, _ref3;
  ...
  build = function(ctx, build_cmd_gen, build_root) {
    var child, cmd_args, opts, was_err;
    cmd_args = ctx.orig_ctx.own_args.comand ? ctx.orig_ctx.own_args.comand.split(' ') : build_cmd_gen(build_root);
    opts = {
      env: process.env,
      cwd: process.env.PWD
    };
    if (!ctx.orig_ctx.own_args.comand) {
      child = spawn(SUB_CAFE, cmd_args, opts);
    } else {
      child = spawn(cmd_args[0], cmd_args.slice(1), opts);
    }
    ...
    return child.on('exit', function(code) {
      ...
      if (was_err === false) {
        if (ctx.orig_ctx.own_args.comand) {
          return ctx.orig_ctx.emitter.emit('NOTIFY_SUCCESS');
        }
      } else {
        if (ctx.orig_ctx.own_args.comand) {
          return ctx.orig_ctx.emitter.emit('NOTIFY_FAILURE');
        }
      }
    });
  };
  ...
  module.exports = {
    watch: function(ctx) {
      ...
      return ctx.fb.say("Watching application changes ...'" + ctx.watch_root + "'\n Press `Ctrl-c` to stop watching.");
    }
  };

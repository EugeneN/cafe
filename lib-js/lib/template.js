        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          if (dir === 'app') {
            if (file === 'index.coffee') {
              file = this.values.name + ".coffee";
            } else if (file === 'index_tests.coffee') {
              file = this.values.name + "Tests.coffee";
            }
          }
    ...
    Template.prototype.write = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (isDir(path)) {
          ...
          if (fd.basename(out) === 'index.coffee') {
            out = fd.dirname(out) + ("/" + this.values.name + ".coffee");
          } else if (fd.basename(out) === 'index_tests.coffee') {
            out = fd.dirname(out) + ("/" + this.values.name + "Tests.coffee");
          }

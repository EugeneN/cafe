...
(function() {
  var COMMON_JS, PLAIN_JS, bootstrapper_file, fs, path, wrap_bundle, wrap_file, wrap_module, wrap_modules;
  ...
  wrap_module = function(sources, ns) {
    ...
    if (sources.length) {
      ...
      if (sources.type === COMMON_JS) {
        return wrap_module([sources], ns);
      } else {
        return ["/*ZB: " + sources.filename + " */", "" + sources.source + ";"].join('\n');
      }
    }
  };
